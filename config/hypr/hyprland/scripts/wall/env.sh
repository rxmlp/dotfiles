...
config_file="$HOME/.config/hypr/hyprland/devices.conf"

monitor=$(hyprctl monitors -j | jq -r '.[] | select(.focused == true) | .description')
monitors=$(hyprctl monitors -j | jq -r '.[] | .description')
monitor_primary_desc=$(grep '^\$monitor_primary = desc:' "$config_file" | sed -E 's/^\$monitor_primary = desc:(.*)$/\1/')
...
monitor_secondary_desc=$(grep '^\$monitor_secondary = desc:' "$config_file" | sed -E 's/^\$monitor_secondary = desc:(.*)$/\1/')
